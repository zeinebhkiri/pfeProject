...
const ManagerAnalyticsPage = () => {
  ...
  // ── Tâches de l'équipe (depuis les parcours de l'équipe) ─────────────────
  const tasks = useMemo<Task[]>(() => {
    const raw = teamParcours as any[];
    const teamMemberIds = new Set((team as User[]).map(u => u.id));
    return raw
      .filter(item => {
        const userId = item.salarie?.id ?? item.parcours?.userId ?? item.userId;
        return teamMemberIds.has(userId);
      })
      .flatMap(item => (item.tasks ?? []) as Task[]);
  }, [teamParcours, team]);
