...
const AdminParcoursTemplatesArchivesPage = () => {
  ...
  const getPositionTitre = (templateId: string) => {
    const position = positions.find((p: any) => p.parcoursTemplateId === templateId);
    return position?.titre || "—";
  };
  ...
  return (
    <div className="flex min-h-screen" style={{ background: "#F8FAFC" }}>
      ...
      💼 {getPositionTitre(template.id)} • {template.description || "Aucune description"}
    </p>
                      </div>
